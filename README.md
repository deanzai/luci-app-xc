...
- **节点测速**：单节点测速或全部测速；并发度可设置为 1–5，默认 3。测速是节点端口/传输层连通性与延迟测试，不等同于真实代理请求
...
| Real connection test URL | 切换真实连接测试及出口 IP 检测地址；旧配置字段仍为 `health_url` | `https://api.ipify.org` |
| Real connection test timeout | 监听就绪和两条真实代理请求共用的总超时时间，范围 1–30 秒；旧配置字段仍为 `health_timeout` | `15` 秒 |
...
1. **设置**：启用插件，检查监听地址、端口、节点测速和真实连接测试参数。
2. **节点**：新增或编辑自建节点；节点列表同行提供测速、快速切换、编辑和删除操作。
3. **快速切换**：对已保存并应用到运行配置的节点，点击节点行的“快速切换”即可在不重启 Xray 的情况下切换；新增或修改节点前先完成配置保存。
4. **完整切换**：在“设置”页修改“活动节点”后点击 LuCI 的“保存并应用”。插件会校验配置、启动 Xray，并经 SOCKS/HTTP 代理各发起一次真实请求；两者成功后才提交节点，否则自动回滚。
...
核心页的“核心资源”区域还提供 Xray、GeoIP、GeoSite 三个独立更新入口。每项可从代码内置的官方源或固定镜像源中选择，
不提供自定义 URL；点击“更新”后直接下载并替换受管资源。此流程不做内容、版本、哈希或 `xray run -test` 语义校验，
更新成功只代表下载和文件替换成功。Xray 下载版本固定为 `26.6.27`，只安装为未激活版本，仍需在已安装版本列表中手动激活。

GeoIP/GeoSite 更新前会各自保存一份初始默认快照，后续更新不会覆盖；页面“回滚”从该快照恢复。资源受管目录为
`/etc/xc/xray/assets/`，优先于系统资源目录，系统 `/usr/share/xray` 和 `/usr/share/v2ray` 文件不会被直接覆盖。

GeoIP/GeoSite 文件由目标发行版的资源包提供，r20 按 `26.6.27` 兼容上限准备资源。插件要求同一资源目录
同时存在 `geoip.dat` 和 `geosite.dat`；资源更新后如需应用新资源，请按页面提示观察运行状态并在必要时重启服务。
资源缺失时不会启动或替换当前配置。
...
状态页的 `Exit IP` 是通过 XC 本地 SOCKS 监听、经当前节点访问真实连接测试 URL 后观察到的公网 IP，不是路由器本地 WAN 地址。结果会在受保护的运行目录中短暂缓存；服务、监听或真实请求不可用时显示 `Unavailable`。
